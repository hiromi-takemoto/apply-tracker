...
type Row = { id: string; platform: ApplicationPlatform; title: string; actual_amount: number | null; status: ApplicationStatus; deadline: string | null };

export default async function ApplicationsPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/login");
  const { data, error } = await supabase.from("applications").select("id, platform, title, actual_amount, status, deadline").order("created_at", { ascending: false });
